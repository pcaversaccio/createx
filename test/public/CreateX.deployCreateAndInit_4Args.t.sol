...
contract CreateX_DeployCreateAndInit_4Args_Public_Test is BaseTest {
        ...
        whenTheNonpayableInitialisationCallIsUnsuccessful
    {
        ...
        createX.deployCreateAndInit{value: msgValue}(cachedInitCode, abi.encodeWithSignature("wagmi"), values, arg3);
    }
    ...
    function testFuzz_WhenTheCreatedRuntimeBytecodeHasAZeroLength(
        ...
        bytes memory expectedErr = abi.encodeWithSelector(CreateX.FailedContractCreation.selector, createXAddr);
        vm.expectRevert(expectedErr);
        createX.deployCreateAndInit{value: msgValue}(new bytes(0), new bytes(0), values, SELF);
    }
    ...
    function testFuzz_WhenTheContractCreationFails(uint64 nonce) external whenTheContractCreationFails {
        ...
        bytes memory expectedErr = abi.encodeWithSelector(CreateX.FailedContractCreation.selector, createXAddr);
        vm.expectRevert(expectedErr);
        createX.deployCreateAndInit{value: msgValue}(invalidInitCode, new bytes(0), values, SELF);
    }
}
