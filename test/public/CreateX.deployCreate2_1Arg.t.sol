...
contract CreateX_DeployCreate2_1Arg_Public_Test is BaseTest {
    ...
    function testFuzz_whenTheInitCodeSuccessfullyCreatesARuntimeBytecodeWithANonZeroLength(
        ...
        (
            ...
            bool mustRevert,
            bytes32 guardedSalt
        ) = parseFuzzerSalt(createXHarness.exposed_generateSalt());

        if (mustRevert) {
            bytes memory expectedErr = abi.encodeWithSelector(CreateX.InvalidSalt.selector, createXAddr);
            vm.expectRevert(expectedErr);
            createX.deployCreate2{value: msgValue}(cachedInitCode);
        } else {
            ...
            if (permissionedDeployProtection && xChainRedeployProtection) {
                vm.chainId(chainId);
                address newContractOriginalCaller = createX.deployCreate2{value: msgValue}(cachedInitCode);
    ...
    function testFuzz_WhenTheInitCodeSuccessfullyCreatesARuntimeBytecodeWithAZeroLength(
        ...
        msgValue = bound(msgValue, 0, type(uint64).max);
        (, , bool mustRevert, ) = parseFuzzerSalt(createXHarness.exposed_generateSalt());
